...
def argparse_init() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-p", "--port", type=int, default=1883, help="port of the MQTT broker"
    )
    parser.add_argument(
        "--tls", action=argparse.BooleanOptionalAction, help="tls authentication"
    )
    tls = parser.add_argument_group(
        "tls", description="if tls is used, set following arguments"
    )

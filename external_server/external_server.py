...
from external_server.checker.session_checker import SessionChecker
...
from external_server.exceptions import (
    ...
    ConnectSessionTimeOutExc,
    NormalCommunicationException,
    StatusTimeOutExc,
    UnexpectedDisconnectionExc,
...
class ExternalServer:
    def __init__(self, ip_address: str, port: int) -> None:
        ...
        self.port = port
        self.session_id = ""
        self.msg_checker = SessionChecker(ConnectSessionTimeOutExc())
        self.command_response_checker = AcknowledgmentChecker(CommandResponseTimeOutExc())
        self.status_order_checker = OrderChecker(StatusTimeOutExc())
    ...
    def start(self) -> None:
        ...
        while True:
            try:
                ...
                self.status_order_checker.reset()
                self.connected_devices.clear()

    def _init_sequence(self) -> None:
        devices_num = self._init_seq_connect()
        self._init_seq_status(devices_num)
        self._init_seq_command(devices_num)
    ...
    def _init_seq_connect(self) -> int:
        ...
        sent_msg = self._create_connect_response(
            received_msg.connect.sessionId, external_protocol.ConnectResponse.Type.OK
        )
    ...
    def _init_seq_status(self, devices_num: int) -> Queue:
        for _ in range(devices_num):
            ...
            self.status_order_checker.check(status_msg.status)
            logging.info(
    ...
    def _init_seq_command(self, devices_num: int) -> None:
        for _ in range(devices_num):
            status_message = self.status_order_checker.get_status()
    ...
    def _normal_communication(self) -> None:
        ...
        while True:
            ...
            if isinstance(received_msg, bool):
                self.mqtt_client.stop()
                raise UnexpectedDisconnectionExc()
            ...
            if received_msg.HasField("connect"):
                ...
                self._handle_command_response(received_msg.commandResponse.sessionId, received_msg.commandResponse.messageCounter)

    def _check_timeouts(self) -> None:
        self.command_response_checker.check_time_out()
        self.msg_checker.check_time_out()
        self.status_order_checker.check_time_out()

    def _handle_connect(self, session_id: str) -> None:
        logging.warning("Received Connect message")
        if self._check_session_id(session_id):
            raise NormalCommunicationException("Connected session sent Connect message")
        sent_msg = self._create_connect_response(session_id, external_protocol.ConnectResponse.Type.ALREADY_LOGGED)
        self.mqtt_client.publish(sent_msg)

    def _handle_status(self, received_status: external_protocol.Status) -> None:
        logging.info(f"Received Status message message, messageCounter: {received_status.messageCounter}")
        self._reset_msg_checker_if_session_id_is_ok(received_status.sessionId)
        self.status_order_checker.check(received_status)

        while (status := self.status_order_checker.get_status()) is not None:
            ...
            match status.deviceState:
                case external_protocol.Status.DeviceState.RUNNING:
                    if not self._check_device_is_connected(device):
                        ...
                        continue
                    self._send_status_response_and_command(status)
                ...
                case external_protocol.Status.DeviceState.DISCONNECT:
                    if not self._check_device_is_connected(device):
                        ...
                        continue
                    sent_msg = self._create_status_response(status)
                    self.mqtt_client.publish(sent_msg)
                    self.connected_devices[device.module][device.deviceType][device.deviceRole] = False

    def _handle_command_response(self, session_id: str, message_counter: int) -> None:
        self._reset_msg_checker_if_session_id_is_ok(session_id)
        self.command_response_checker.remove_ack(message_counter)
    ...
    def _create_connect_response(self, session_id: str, connect_response_type: int) -> external_protocol.ExternalServer:
        ...
        logging.info(f"Sending Connect response message, response type: {connect_response_type}")
        return sent_msg
    ...
    def _create_status_response(self, status: external_protocol.Status) -> external_protocol.ExternalServer:
        ...
        sent_msg.statusResponse.CopyFrom(
            self.msg_creator[module].create_status_response(status.sessionId, status.messageCounter)
        )
        logging.info(f"Sending Status response message, messageCounter: {status.messageCounter}")
        return sent_msg
    ...
    def _create_command(self, status: external_protocol.Status) -> external_protocol.ExternalServer:
        ...
        sent_msg.command.CopyFrom(
            self.msg_creator[module].create_external_command(status.sessionId, command_counter, status.deviceStatus)
        )
    ...
    def _send_status_response_and_command(self, status: external_protocol.Status) -> None:
        sent_msg = self._create_status_response(status)
        self.mqtt_client.publish(sent_msg)
        sent_msg = self._create_command(status)
        self.mqtt_client.publish(sent_msg)

...
class ExternalServer:
    ...
    def _init_seq_connect(self) -> int:
        ...
        for device in devices:
            self.connected_devices[device.module][device.deviceType][
                device.deviceRole
            ] = True

        sent_msg = self._create_connect_response(
            self.session_id, external_protocol.ConnectResponse.Type.OK
        )
    ...
    def _init_seq_status(self, devices_num: int) -> Queue:
        ...
        for _ in range(devices_num):
            ...
            if not self._check_device_is_connected(device):
                ...
                raise ConnectSequenceException()
            if not self._check_status_device_state(
                status_msg, external_protocol.Status.DeviceState.CONNECTING
            ):
                logging.error(
    ...
    def _init_seq_command(self, devices_num: int, received_statuses: Queue) -> None:
        ...
        for _ in range(devices_num):
            ...
            if not received_msg.HasField("commandResponse"):
                ...
                raise ConnectSequenceException()
            self.command_response_checker.remove_ack(
    ...
    def _normal_communication(self) -> None:
        ...
        while True:
            ...
            if self.command_response_checker.check_time_out():
                raise NormalCommunicationException(
                    "Command response message has not been recieved in time"
                )
            if self.msg_checker.check_time_out():
                raise NormalCommunicationException(
                    "Connected session has been timed out"
                )
            ...
            if received_msg.HasField("connect"):
                ...
                if self._check_session_id(received_msg.connect.sessionId):
                    raise NormalCommunicationException(
                        "Connected session sent Connect message"
                    )
                sent_msg = self._create_connect_response(
                ...
                logging.info(
                    f"Received Status message message, messageCounter: {received_msg.status.messageCounter}"
                )
                self._reset_msg_checker_if_session_id_is_ok(
                ...
                self._reset_msg_checker_if_session_id_is_ok(
                    received_msg.commandResponse.sessionId
                )
                self.command_response_checker.remove_ack(
                    received_msg.commandResponse.messageCounter
                )
    ...
    def _create_connect_response(
        self, session_id: str, connect_response_type: int
    ) -> external_protocol.ExternalServer:
        connect_response = self.msg_creator[
            ModuleType.MISSION_MODULE.value
        ].create_connect_response(session_id, connect_response_type)
    ...
    def _create_status_response(
        ...
        sent_msg.statusResponse.CopyFrom(
            self.msg_creator[module].create_status_response(
                status.status.sessionId, status.status.messageCounter
            )
        )
        logging.info(
            f"Sending Status response message, messageCounter: {status.status.messageCounter}"
        )
        return sent_msg
    ...
    def _check_device_is_connected(self, device) -> bool:
        return self.connected_devices[device.module][device.deviceType][
            device.deviceRole
        ]
